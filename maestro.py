...
In dit geval zal ik gebruik maken van de 
'''

# allemogelijke opties genereren
mogelijke_gokken = list(itertools.product(range(1, 7), repeat=4))
...
def analyse(ronde, answers, gokken):
    ...
    for i in gokken[ronde]:
        tmp_code = [i for i in mogelijke_gokken]
        if i in tmp_code:
            if gokken[ronde].index(i) == mogelijke_gokken.index(i):
